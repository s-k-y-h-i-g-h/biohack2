...
pub fn seed_catalog() -> Vec<CatalogItem> {
    vec![
        ...
        make_item(
            ...
            "24h",
            &[],
            &["Consult physician if taking thiazide diuretics"],
        ),
        make_item(
            "Vitamin K2",
            ItemType::Supplement,
        ...
        make_item(
            ...
            "8h",
            &[],
            &["May interact with antidepressants (serotonergic)"],
        ),
        make_item(
            "Cordyceps",
            ItemType::Supplement,
...
mod tests {
    ...
    fn test_all_items_have_dosage_range() {
        ...
        for item in &catalog {
            assert!(
                item.dosage_range.is_some(),
                "Item {} missing dosage_range",
                item.name
            );
    ...
    fn test_known_interactions_present() {
        ...
        assert!(
            omega
                .contraindications
                .iter()
                .any(|c| c == "blood-thinners")
        );

        let vitk2 = catalog
            .iter()
            .find(|i| i.name.contains("Vitamin K2"))
            .unwrap();
        assert!(
            vitk2
                .contraindications
        ...
        let berberine = catalog
            .iter()
            .find(|i| i.name.contains("Berberine"))
            .unwrap();
