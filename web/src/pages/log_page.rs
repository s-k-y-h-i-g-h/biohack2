...
pub fn LogPage() -> impl IntoView {
    ...
    let selected_item = RwSignal::new(None::<crate::pages::CatalogItem>);
    ...
    let handle_select = move |item: crate::pages::CatalogItem| {
    ...
    let handle_save = move |_| {
        ...
        if let Some(item) = selected_item.get() {
            let item_type = match item.category.as_str() {
            ...
            match create_log_entry(&LogEntry {
                id: uuid::Uuid::new_v4(),
                        ...
                        filtered_catalog().into_iter().map(|item| {
                            ...
                            view! {
                                <button
                                    type="button"
                                    class=move || {
                                        if selected_item.get().as_ref().map(|s| s.name == name_for_closure).unwrap_or(false) {
...
fn seed_catalog() -> Vec<crate::pages::CatalogItem> {
    crate::pages::catalog::seed_catalog()
}
