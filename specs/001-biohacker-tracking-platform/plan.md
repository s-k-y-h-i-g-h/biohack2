...
**Language/Version**: TypeScript 5.x + SolidJS (web); Rust 2024 (shared engine)

**Primary Dependencies**: 
- Frontend Web: TypeScript + SolidJS (reactive, local-first patterns)
- Backend: Rust 2024 (shared engine with biohack CLI)
- Database: SQLite (local via `sql.js` or native WASM binding)
- Cloud sync: Deferred to v2 (OAuth 2.0 / OIDC, encrypted PostgreSQL)

**Storage**: 
- Local: SQLite via WASM (offline-capable, IndexedDB fallback)
...
**Testing**: TypeScript + Vitest (web); Rust test suite (backend/safety engine — reuse from biohack); cloud/mobile tests deferred to v2
...
- Push notification latency: <30 seconds from server to device (v2 cloud)
...
- Local storage via IndexedDB/WASM SQLite
...
|| V. Modular Architecture | ✅ | Clear separation: backend engine (Rust), web frontend (TypeScript), sync service (deferred), mobile frontends (deferred) |
...
**Structure Decision**: Hybrid architecture with Rust engine (reused from biohack CLI) for safety-critical logic, TypeScript web app for v1. Cloud sync and mobile frontends are separate optional modules deferred to v2. This aligns with Constitution V (modular architecture for multiple frontends).
